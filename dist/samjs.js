...
(function (global, factory) {
  ...
  */
  ...
  let russianPhonemes = {
    ...
    'я': 'Y*AA',
    'Я': 'Y*AA',
    // as in "yard" - using Y* + AA
    'о': 'OW',
    'О': 'OW',
    // as in "go"
    'ё': 'Y*OW',
    'Ё': 'Y*OW',
    // as in "you" - using Y* + OW
    'у': 'UW',
    'У': 'UW',
    // as in "boot"
    'ю': 'Y*UW',
    'Ю': 'Y*UW',
    // as in "you" - using Y* + UW
    'э': 'EH',
    'Э': 'EH',
    // as in "bed"
    'е': 'Y*EH',
    'Е': 'Y*EH',
    // as in "yes" - using Y* + EH
    'и': 'IY',
    'И': 'IY',
    // as in "see"
    'ы': 'IH',
  ...
  let russianSettings = {
    pitch: 64,
    // Slightly lower pitch for Russian
    speed: 72,
    // Standard speed
    mouth: 128,
    // Standard mouth
  ...
  let convertToPhonemes = function (text) {
    ...
    if (lang.phonemes) {
      ...
      let entries = Object.entries(lang.phonemes).sort((a, b) => b[0].length - a[0].length);
  ...
  let getLanguageSettings = function () {
  ...
  function SamJs(options) {
    ...
    let langSettings = getLanguageSettings(language);
    ...
    let ensurePhonetic = (text, phonetic) => {
      ...
      if (!(phonetic || opts.phonetic)) {
        ...
        if (language !== 'en') {
          try {
            let result = convertToPhonemes(text, language);
    ...
    this.buf8 = (text, phonetic) => {
      let phonemes = ensurePhonetic(text, phonetic);
